...
class plgSystemHikashopsocial extends hikashopJoomlaPlugin {
	...
	function onAfterRender() {
		...
		$html = array(
			$this->_addTwitterButton($plugin, $plugin->params),
			$this->_addPinterestButton($plugin, $plugin->params),
			$this->_addFacebookButton($plugin, $plugin->params),
			$this->_addLinkedInButton($plugin, $plugin->params)
		);
...
(function(d,s,id) {
	...
	function _addFacebookButton(&$plugin, $params) {
		if ($params['legacy']) {
			...
			$return =  ''.
			'<script async defer crossorigin="anonymous" src="https://connect.facebook.net/fr_FR/sdk.js#xfbml=1&version=v17.0" nonce="UpVDUOjt"></script>'.
			$btn_html;
		}
		return $return;
	}
