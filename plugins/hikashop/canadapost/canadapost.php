...
<tr>
	...
	protected function loadOptions() {
		...
		if(!isset($this->params)) {
			...
			$plugin = $pluginsClass->getByName('hikashop', 'canadapost');
	...
	public function onBeforeDisplayShippingMethod(&$shippingMethods, $order) {
        // Check if the "Add Shipping Setting" button was clicked
        $addShippingSetting = JFactory::getApplication()->input->get('add_shipping_setting', 0, 'int');

		echo $addShippingSetting;

        // if ($addShippingSetting) {
        //     $params = $this->params->toArray();

        //     // Get the current shipping settings
        //     $currentSettings = $params['default_shipping_settings'];

        //     // Generate a unique field name for the new setting
        //     $newFieldName = 'default_shipping_settings_' . (count($currentSettings) + 1);

        //     // Add the new setting with default values
        //     $currentSettings[$newFieldName] = 0; // Default value

        //     // Update the params with the new settings
        //     $params['default_shipping_settings'] = $currentSettings;
	...
	function addShippingSetting($params, $field_name) {
		...
		// Generate a unique field name for the new setting
		$new_field_name = 'default_shipping_settings_' . (count($current_settings) + 1);

		// Add the new setting with default values
		$current_settings[$new_field_name] = 0; // Default value

		// Update the params with the new settings
		$params[$field_name] = $current_settings;
